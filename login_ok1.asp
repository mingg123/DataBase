...
<%
  ...
  sql="select 학부모.학부모회원id , 회원.비밀번호 from 학부모, 회원 where 학부모.학부모회원id=회원.id"
  ...
  loop
   ...
   if tt2=false then
	Response.Write "<script>alert('학생 회원으로 로그인해주세요.');history.back(-1);</script>"
   
	else
 
  session("ID")=ID
  if tt = true then
	Response.Write "<script>alert(학부모회원님 반갑습니다.)</script>"
   response.redirect("학부모login.asp")
  else
